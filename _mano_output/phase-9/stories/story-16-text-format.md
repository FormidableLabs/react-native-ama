### STORY-16: Text page format

#### What and why
A developer reading about text accessibility expects guidance on making text content readable and correctly announced by screen readers, including font size, line spacing, and dynamic type. The current `text.md` page predates the format standard and lacks the structured AT expectations and consistent section layout present in `accessibility-label.md`. This story updates the page to match that format.

#### Done when
- [ ] `website/guidelines/text.md` includes the frontmatter fields: `ama_severity`, `ama_category`, `ama_affected_users`, `ama_success_criterion`, and `displayed_sidebar: guidelines`
...
- **Files:** `website/guidelines/text.md` (target); `website/guidelines/accessibility-label.md` (read-only format model)
