### STORY-8: Contrast page format

#### What and why
A developer reading about contrast expects guidance on meeting colour contrast requirements for text and interactive elements so that users with low vision can read content. The current `contrast.md` page predates the format standard and lacks the structured AT expectations and consistent section layout present in `accessibility-label.md`. This story updates the page to match that format.

#### Done when
- [ ] `website/guidelines/contrast.md` includes the frontmatter fields: `ama_severity`, `ama_category`, `ama_affected_users`, `ama_success_criterion`, and `displayed_sidebar: guidelines`
...
- **Files:** `website/guidelines/contrast.md` (target); `website/guidelines/accessibility-label.md` (read-only format model)
