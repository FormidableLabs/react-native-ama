### STORY-9: Focus page format

#### What and why
A developer reading about focus management expects guidance on controlling focus order and visibility so keyboard and switch access users can navigate without getting lost. The current `focus.md` page predates the format standard and lacks the structured AT expectations and consistent section layout present in `accessibility-label.md`. This story updates the page to match that format.

#### Done when
- [ ] `website/guidelines/focus.md` includes the frontmatter fields: `ama_severity`, `ama_category`, `ama_affected_users`, `ama_success_criterion`, and `displayed_sidebar: guidelines`
...
- **Files:** `website/guidelines/focus.md` (target); `website/guidelines/accessibility-label.md` (read-only format model)
