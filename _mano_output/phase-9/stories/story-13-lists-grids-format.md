### STORY-13: Lists and grids page format

#### What and why
A developer reading about lists and grids expects guidance on making list content navigable and each item properly announced to screen reader users. The current `lists-grids.md` page predates the format standard and lacks the structured AT expectations and consistent section layout present in `accessibility-label.md`. This story updates the page to match that format.

#### Done when
- [ ] `website/guidelines/lists-grids.md` includes the frontmatter fields: `ama_severity`, `ama_category`, `ama_affected_users`, `ama_success_criterion`, and `displayed_sidebar: guidelines`
...
- **Files:** `website/guidelines/lists-grids.md` (target); `website/guidelines/accessibility-label.md` (read-only format model)
