### STORY-3: Accessibility role page format

#### What and why
A developer reading about accessibility roles expects guidance on communicating element purpose and interaction type to assistive technology users. The current `accessibility-role.md` page predates the format standard and lacks the structured AT expectations and consistent section layout present in `accessibility-label.md`. This story updates the page to match that format.

#### Done when
- [ ] `website/guidelines/accessibility-role.md` includes the frontmatter fields: `ama_severity`, `ama_category`, `ama_affected_users`, `ama_success_criterion`, and `displayed_sidebar: guidelines`
...
- **Files:** `website/guidelines/accessibility-role.md` (target); `website/guidelines/accessibility-label.md` (read-only format model)
