### STORY-17: Timed actions page format

#### What and why
A developer reading about timed actions expects guidance on making time-limited interactions accessible so that users who need more time — due to motor impairments or cognitive differences — can still complete them. The current `timed-actions.md` page predates the format standard and lacks the structured AT expectations and consistent section layout present in `accessibility-label.md`. This story updates the page to match that format.

#### Done when
- [ ] `website/guidelines/timed-actions.md` includes the frontmatter fields: `ama_severity`, `ama_category`, `ama_affected_users`, `ama_success_criterion`, and `displayed_sidebar: guidelines`
...
- **Files:** `website/guidelines/timed-actions.md` (target); `website/guidelines/accessibility-label.md` (read-only format model)
