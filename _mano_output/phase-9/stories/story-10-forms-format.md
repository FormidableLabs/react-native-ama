### STORY-10: Forms page format

#### What and why
A developer reading about forms expects guidance on making input fields, validation errors, and submission accessible to screen reader and keyboard users. The current `forms.md` page predates the format standard and lacks the structured AT expectations and consistent section layout present in `accessibility-label.md`. This story updates the page to match that format.

#### Done when
- [ ] `website/guidelines/forms.md` includes the frontmatter fields: `ama_severity`, `ama_category`, `ama_affected_users`, `ama_success_criterion`, and `displayed_sidebar: guidelines`
...
- **Files:** `website/guidelines/forms.md` (target); `website/guidelines/accessibility-label.md` (read-only format model)
