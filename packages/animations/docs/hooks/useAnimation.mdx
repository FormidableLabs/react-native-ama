...
When reduce Motion is off, the animation is played as specified; while when is **on** any motion animation is played instantaneously (using a duration of 0), while other properties, like opacity, are played as specified.
